...
export default class HelpDesk {
  ...
  addItem() {
    ...
    const createRequest = this.request.createTicket(this.modal.inputName.value, this.modal.inputDescription.value, 0);
    createRequest.then(() => {
      this.showTable();
    });
  }
  ...
  editItem(item) {
    ...
    const createRequest = this.request.editTicket(item.id, this.modal.inputName.value, this.modal.inputDescription.value);
    createRequest.then(() => {
      this.showTable();
    });
  }
